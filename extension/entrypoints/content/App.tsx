...
function applyCss(css: string) {
  let style = document.getElementById('frimmy-style') as HTMLStyleElement | null;
  if (!style) {
...
export default function App({ host }: { host: string }) {
  ...
  const [selected, setSelected] = useState<{ selector: string; html: string } | null>(null);
  ...
  useEffect(() => {
    const picker = new ElementPicker({
      ...
      onClick: (target) => {
        setSelected({ selector: cssPath(target), html: target.outerHTML.slice(0, 4000) });
        picker.stopPicking();
      },
    });
  ...
  async function send() {
    ...
    if (!selected) {
      setMessages((m) => [...m, { role: 'error', text: 'Pick an element first (click the extension icon).' }]);
      return;
    }
    ...
    const res = await browser.runtime.sendMessage({ type: 'ai-edit', prompt, context });
    setBusy(false);
    if (res?.error || typeof res?.css !== 'string') {
      // keep the prompt recoverable: drop it back in the box so a typo is one edit away
      setMessages((m) => [...m, { role: 'error', text: res?.error ?? 'No CSS returned.' }]);
      setInput(prompt);
      return;
    }
    applyCss(res.css);
    setMessages((m) => [...m, { role: 'assistant', text: 'Applied.', css: res.css }]);
  }
  ...
  async function save() {
    ...
    setMessages((m) => [
      ...m,
      res?.id
        ? { role: 'assistant', text: `Saved. Share: ${location.origin}?frimmy=${res.id}` }
  ...
  return (
    <div className="frimmy">
      <div className="frimmy-head">
        Frimmy
        <span className="frimmy-target">{selected ? selected.selector : 'no element'}</span>
      </div>
      ...
      <div className="frimmy-actions">
        <button onClick={send} disabled={busy}>Generate</button>
        <button onClick={save} disabled={busy}>Save & Share</button>
      </div>
    </div>
  );
}
